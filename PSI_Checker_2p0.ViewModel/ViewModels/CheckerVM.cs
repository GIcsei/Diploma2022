...
{
    public class CheckerVM : PlotterVM
    {
        // TODO
        /*
        ...
        #region Commands
        public ICommand PowerOn { get; }
        private async Task ExecutePowerOn()
        {
            byte command = CreateDigitalCommand();
            if (IsPowered)
            {
                await device.StopThread();
            }
            else
            {
                await device.SendDigitalImmediate(command);
                await device.SetPattern(SelectedPattern);
                await Task.Delay(2000);
                await scope.StartThread();
                await device.StartThread();
                await Task.Delay(3000);
                await scope.StopThread();
                await ShowAll(scope.FileSaver.LoadAllData());
            }
            IsPowered = !IsPowered;
        }

        public ICommand LoadPatternFile { get; }
        private async Task ExecuteLoadPatternFile()
        {
            OpenFileDialog openFileDialog = new OpenFileDialog
            {
                Filter = "Config files (*.txt)|*.txt"
            };
            ...
            var loader = new Pattern();
            var result = await loader.PopulatePattern(new FileInfo(PatternFilePath));
            Patterns = await Pattern.LoadAllPatterns(new FileInfo(PatternFilePath));
            if (result)
            {
                DIService.Instance.Get<ImportantInfos>().CurrentPatternName = loader.Name;
        ...
        public CheckerVM()
        {
            ...
            LoadPatternFile = new RelayCommand(async () => await ExecuteLoadPatternFile());
            ...
            PowerOn = new RelayCommand(async () => await ExecutePowerOn());
        ...
        private void InitBoardSettings()
        {
            ...
            Bypass = true;
            Gain = true;
            boardSettings[7] = true;
            boardSettings[2] = false;
        }
        ...
        private byte CreateDigitalCommand()
        {
            byte command = 0;
            for (int i = 0; i < 8; i++)
            {
                command |= (byte)((boardSettings[i] ? 1 : 0) << i);
            }
            return command;
        }
    }
