...
namespace PSI_Checker_2p0.Utils.Async
{
    /// <summary>
    /// Adds the ability to safely await on tasks to be complete that need limited access
    /// For example, only allowing one AnalogTask to access some data at a time, like the old 
    /// asynchronous locks
    /// 
    ...
    public static class AsyncAwaiter
    {
        ...
        /// <summary>
        /// Awaits for any outstanding tasks to complete that are accessing the same key then runs the given AnalogTask, returning it's value
        /// </summary>
        /// <param name="key">The key to await</param>
        /// <param name="task">The AnalogTask to perform inside of the semaphore lock</param>
        /// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access this AnalogTask before it waiting</param>
        /// <returns></returns>
        public static async Task<T> AwaitResultAsync<T>(string key, Func<Task<T>> task, int maxAccessCount = 1)
        {
            ...
            {
                //
                // When the AnalogTask is ready, release the semaphore
                //
                ...
                // Program execution may crash or take a different patternFilePath, this way you are guaranteed execution
                //
                SelfLock.Release();
            }
            ...
            // Now use this semaphore and perform the desired AnalogTask inside its lock
        ...
        /// <summary>
        /// Awaits for any outstanding tasks to complete that are accessing the same key then runs the given AnalogTask
        /// </summary>
        /// <param name="key">The key to await</param>
        /// <param name="task">The AnalogTask to perform inside of the semaphore lock</param>
        /// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access this AnalogTask before it waiting</param>
        ...
        public static async Task AwaitAsync(string key, Func<Task> task, int maxAccessCount = 1)
        {
            ...
            catch (Exception ex)
            {
                ...
                //DIService.Logger.Log($"Crash in {nameof(AwaitAsync)}. {ex.Message}", LogLevel.Debug);
